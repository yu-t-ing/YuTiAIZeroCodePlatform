...
import com.yuti.yutiaicodemother.mapper.ChatHistoryMapper;
...
public class ChatHistoryServiceImpl extends ServiceImpl<ChatHistoryMapper, ChatHistory> implements ChatHistoryService {
    ...
    // 消息最大长度限制（LONGTEXT 理论上最大 4GB，但设置一个合理的上限）
    // 设置为 10MB（10 * 1024 * 1024 字符），足以存储大型代码生成结果
    ...
    public boolean addChatMessage(Long appId, String message, String messageType, Long userId) {
        ...
        // 检查消息长度，如果超过限制，则截断并添加提示
        String messageToSave = message;
        if (message.length() > MAX_MESSAGE_LENGTH) {
            log.warn("消息长度超过限制，appId: {}, 原始长度: {}, 限制: {}, 将进行截断",
                    appId, message.length(), MAX_MESSAGE_LENGTH);
            messageToSave = message.substring(0, MAX_MESSAGE_LENGTH) +
                    "\n\n... [消息过长，已截断。原始长度: " + message.length() + " 字符]";
        }
    ...
    public int loadChatHistoryToMemory(Long appId, MessageWindowChatMemory chatMemory, int maxCount) {
        try {
            ...
            // Java 17 兼容方式：创建新列表并反转
            List<ChatHistory> reversedList = new ArrayList<>(historyList);
            Collections.reverse(reversedList);
            historyList = reversedList;
            // 按照时间顺序将消息添加到记忆中
            int loadedCount = 0;
            // 先清理历史缓存，防止重复加载
            chatMemory.clear();
