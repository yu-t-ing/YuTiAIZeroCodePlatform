...
import com.yuti.yutiaicodemother.core.AiCodeGeneratorFacade;
import com.yuti.yutiaicodemother.core.builder.VueProjectBuilder;
import com.yuti.yutiaicodemother.core.handler.StreamHandlerExecutor;
...
import com.yuti.yutiaicodemother.model.dto.app.AppAddRequest;
import com.yuti.yutiaicodemother.model.dto.app.AppQueryRequest;
import com.yuti.yutiaicodemother.model.entity.App;
import com.yuti.yutiaicodemother.mapper.AppMapper;
import com.yuti.yutiaicodemother.model.entity.User;
import com.yuti.yutiaicodemother.model.enums.ChatHistoryMessageTypeEnum;
import com.yuti.yutiaicodemother.model.enums.CodeGenTypeEnum;
import com.yuti.yutiaicodemother.model.vo.AppVO;
import com.yuti.yutiaicodemother.model.vo.UserVO;
import com.yuti.yutiaicodemother.monitor.MonitorContext;
import com.yuti.yutiaicodemother.monitor.MonitorContextHolder;
import com.yuti.yutiaicodemother.service.AppService;
import com.yuti.yutiaicodemother.service.ChatHistoryService;
import com.yuti.yutiaicodemother.service.ScreenshotService;
import com.yuti.yutiaicodemother.service.UserService;
...
public class AppServiceImpl extends ServiceImpl<AppMapper, App> implements AppService {
    ...
    public Flux<String> chatToGenCode(Long appId, String message, User loginUser) {
        ...
        log.info("📌 应用 {} 的代码生成类型（原始值）: {}", appId, codeGenType);

        CodeGenTypeEnum codeGenTypeEnum = CodeGenTypeEnum.getEnumByValue(codeGenType);
        if (codeGenTypeEnum == null) {
            log.error("❌ 无法解析代码生成类型: {}", codeGenType);
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "应用代码生成类型错误");
        }

        log.info("✅ 应用 {} 的代码生成类型（解析后）: {}", appId, codeGenTypeEnum);
    ...
    public Long createApp(AppAddRequest appAddRequest, User loginUser) {
        ...
        if (StrUtil.isBlank(codeGenType)) {
            // 如果用户没有指定类型，使用 AI 智能选择
            log.info("用户未指定代码生成类型，使用 AI 智能路由");
            AiCodeGenTypeRoutingService aiCodeGenTypeRoutingService = aiCodeGenTypeRoutingServiceFactory.createAiCodeGenTypeRoutingService();
            CodeGenTypeEnum selectedCodeGenType = aiCodeGenTypeRoutingService.routeCodeGenType(initPrompt);
            codeGenType = selectedCodeGenType.getValue();
        } else {
            // 用户已指定类型，直接使用
            log.info("用户指定代码生成类型: {}", codeGenType);
        }
    ...
    public String deployApp(Long appId, User loginUser) {
        ...
        if (codeGenTypeEnum == CodeGenTypeEnum.VUE_PROJECT) {
            log.info("📦 开始部署 Vue 项目，路径: {}", sourceDirPath);
            // Vue 项目需要构建
            boolean buildSuccess = vueProjectBuilder.buildProject(sourceDirPath);
            if (!buildSuccess) {
                log.error("❌ Vue 项目构建失败: {}", sourceDirPath);
                throw new BusinessException(ErrorCode.SYSTEM_ERROR, "Vue 项目构建失败，请重试或检查项目代码");
            }
            // 检查 dist 目录是否存在
            File distDir = new File(sourceDirPath, "dist");
            if (!distDir.exists() || !distDir.isDirectory()) {
                log.error("❌ Vue 项目构建完成但 dist 目录不存在: {}", sourceDirPath);
